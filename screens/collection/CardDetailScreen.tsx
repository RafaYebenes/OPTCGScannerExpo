...
export const CardDetailScreen = ({ route, navigation }: any) => {
    ...
    const handleQuantity = async (delta: number) => {
        ...
        if (newQty <= 0) {
            Alert.alert("¿Eliminar carta?", "La cantidad llegará a 0 y se borrará de tu colección.", [
                { text: "Cancelar", style: "cancel" },
                {
                    text: "Eliminar", style: "destructive",
                    onPress: async () => {
                        setUpdating(true);
                        const ids: string[] = item.ids ?? [item.id];
        ...
        try {
            const baseId = item.ids?.[0] ?? item.id;
            // ✅ FIX: usar `qty` (state actual) como base, NO item.quantity (inmutable)
            await updateQuantity(baseId, qty + delta);
        } catch (e) {
            setQty(qty); // rollback
